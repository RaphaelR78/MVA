...
class MessagePassing(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        ##################
        x_node = self.fc1(x) #f-c layer 1
        x_nbrs = self.fc2(x) #f-c layer 2
...
class GNN(nn.Module):
    ...
    def forward(self, x, adj, idx):
        ...
        if self.readout == 'sum':
            ...
            #Somme pour chaque graphe les représentations vectorielles de leur noeud
            #Similairement à Figure 1
        ...
        ##################
        out = self.fc(out)
        #output: vector representation for each input graph
        #size: (n_graphs,output_dim)
        ##################
